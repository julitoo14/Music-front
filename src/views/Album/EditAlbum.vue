<template>
    <div class="container">
      ...
      <form class="mt-5" @submit.prevent="submitForm">
        ...
        <div class="form-group">
          ...
          />
          <button @click="update(album.title, album.description)" type="submit" class="m-3 btn btn-primary">Edit</button>
          <button class="m-3 btn btn-primary"><RouterLink class="nav-link" :to="`/album/${route.params.id}`">Back</RouterLink></button>
        </div>
      </form>
  ...
  const upload = async () => {
    if(file0.value){
        ...
        await axios
        .put(`http://localhost:3910/api/album/upload/${route.params.id}`, formData, {
            headers: {
                Authorization: `${token}`,
  ...
  const update = async (title, description) => {
    ...
    const token = localStorage.getItem("token");
    try{
      const res = await axios.put(`http://localhost:3910/api/album/update/${route.params.id}`,{ title, description },{headers: {Authorization: `${token}`}})
