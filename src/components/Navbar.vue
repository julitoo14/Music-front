<template>
    <div class="sidebar d-flex flex-column flex-shrink-0">
        <RouterLink class="d-flex align-items-center p-3 mb-3 mb-md-0 me-md-auto link-light text-decoration-none flex-wrap justify-content-center" to="/">
        ...
        <ul v-else class="p-3 nav nav-pills flex-column mb-auto">
            <li>
                <RouterLink class="link d-inline-flex nav-link link-light p-1 align-items-center" to="/login">
                    <LoginIcon/> 
                    <p class="m-1">Sign In</p>                 
                </RouterLink>
            </li>
            <li>
                <RouterLink class="link d-inline-flex nav-link link-light p-1 align-items-center" to="/register">
                    <LibraryIcon/> 
        ...
        <div v-if="logged" class="dropdown p-4">
            ...
            <ul class="dropdown-menu shadow bg-dark" aria-labelledby="dropdownUser">
                <li>
                    <RouterLink class="link-light dropdown-item" :to="`/profile/${id}`">Profile</RouterLink>
                </li>
                <li>
                    <RouterLink class="link-light dropdown-item" to="/settings/">Settings</RouterLink>
                </li>
                <li>
                ...
                <li>
                    <RouterLink class="link-light dropdown-item" @click="signOut" to="/">sign Out</RouterLink>
                </li>
            </ul>
        </div>
    </div>
...
onMounted(async () =>{
    ...
    logged.value = true;
    await axios
    .get(`http://localhost:3910/api/user/avatar/${localStorage.getItem('image')}`, {
      headers: {
        Authorization: `${token}`,
      },
